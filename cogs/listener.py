...
class Listener(commands.Cog):
    ...
    async def update_member_bot_count_channel_name(self, guild):
        for_members = self.get_guild_member_count_channel_id(guild)

        member_count_channel_id = for_members[0]
        member_count_suffix = for_members[1]

        for_bots = self.get_guild_bot_count_channel_id(guild)

        bot_count_channel_id = for_bots[0]
        bot_count_suffix = for_bots[1]
        ...
        if member_count_channel_id is not None and member_count_suffix is not None:
            ...
            new_name = f"{member_count_suffix}{Listener.get_guild_count(guild)[0]}"
            print(Listener.get_guild_count(guild)[0])
        ...
        if bot_count_channel_id is not None and bot_count_suffix is not None:
            ...
            new_name = f"{bot_count_suffix}{Listener.get_guild_count(guild)[1]}"
            print(Listener.get_guild_count(guild)[1])
